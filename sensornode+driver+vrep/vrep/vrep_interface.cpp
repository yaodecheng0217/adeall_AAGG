 ...
 * @LastEditTime: 2020-04-09 15:55:39
...
void vrep_interface::connect()
{
    ...
    std::string ip="192.168.2.16";
    int port=19999;
    config.GetValue("simulation_ip",ip);
    config.GetValue("simulation_port",port);
    simxFinish(clientID);
    clientID = simxStart(ip.c_str(), port, true, true, 1300, 5);
    if (clientID != -1)
    {
...
void vrep_interface::init()
{
    while (1)
    {
        ...
        printf("simlastion is not connect! ....\n");
        Sleep(1000);
    }
}
...
int vrep_interface::GetAllData(vrep_data &d)
{
    float temp[3];
    if (clientID == -1)
        return 1;
    int c = simxGetJointPosition(clientID, _handle.Side, &d.side_lasser, simx_opmode_buffer); //侧移量，激光测距数据0
    if (c == 1)
    {
        //clientID = -1;
        return c;
